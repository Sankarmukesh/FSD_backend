...
exports.addTask = async (req, res, next) => {
    try {
        ...
        await send_mail(fetchingNewTask.owner.email, 'Assigned a Task for you!', `A Task <b>${fetchingNewTask.name}</b> has been assigned to you from project <b>${projectDetails.name}</b> and userStory <b>${userStorydetails.name}</b>`)
...
exports.updateTask = async (req, res, next) => {
    try {
        ...
        if (beforeUpdateTask?.owner?._id.toString() !== updatedTask?.owner?._id.toString()) {
            await send_mail(updatedTask.owner.email, 'Assigned a Task for you!', `A Task <b>${updatedTask.name}</b> has been assigned to you from project <b>${projectDetails.name}</b> and userStory <b>${userStorydetails.name}</b>`)
        } else {
            await send_mail(updatedTask.owner.email, 'Updates on your task!', `A Task <b>${updatedTask.name}</b> has been updated in project <b>${projectDetails.name}</b> and userStory <b>${userStorydetails.name}</b>`)
        }
